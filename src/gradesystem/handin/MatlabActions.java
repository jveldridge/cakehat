...
class MatlabActions implements ActionProvider
{
    ...
    //TODO: Demo action(s). Definitely have action that runs a specified m-file.
    //Have a demo mode that allows for selecting which file m-file to run?
    ...
    private class RunFile implements DistributableActionDescription
    {
        ...
        public String getDescription()
        {
            return "Runs a single m-file with grader provided arguments. The " +
                    "grader will be allowed to select which m-file to run from " +
                    "a list of all m-files that are part of this distributable " +
                    "part.";
        }
            ...
            DistributableAction action = new StandardDistributableAction()
            {
                public void performAction(DistributablePart part, Group group) throws ActionException
                {
                    ...
                    for(File mFile : mFiles)
                    {
                        ...
                        relativePath = relativePath.replace(unarchiveDir.getAbsolutePath(), "");
                    ...
                    final Runnable updateEval = new Runnable()
                    {
                        public void run()
                        {
                            ...
                            String evalCmd = fileToFunctionName(mFile);
    ...
    private String fileToFunctionName(File mfile)
    {
