...
class RubricPanel extends JPanel
{
    ...
    //NEW - Entirely from @Override version
    ...
    private void displaySection(Section section)
    {
        ...
        for(Subsection subsection : section.Subsections)
        {
            ...
            {
                scoreField = NumberField.getAsScore(subsection);
            }
            layout.putConstraint(SpringLayout.WEST, scoreField, 5, SpringLayout.EAST, subsectionName);
            layout.putConstraint(SpringLayout.NORTH, scoreField, 0, SpringLayout.NORTH, subsectionName);
            panel.add(scoreField);
    ...
    */
    ...
    private void displayExtraCredit()
    {
        ...
        //TODO: Consider specifying in either rubric or config whether late handins get extra credit
        boolean editable = !TimeStatus.getStatus(_rubric.Status).isLate();

        //For each ExtraCredit section
        for(ExtraCredit extraCredit : _rubric.ExtraCredit)
	{
            //Section name
            ...
            {
                scoreField = NumberField.getAsUneditable(0);
            }
    ...
    private void displayExtraCredit(Rubric rubric)
    {
        ...
        for(ExtraCredit extraCredit : rubric.ExtraCredit)
        {
            ...
            if(editable)
            {
                scoreField = NumberField.getAsScore(extraCredit);
            }
            else
            {
                scoreField = NumberField.getAsUneditable(0.0);
    ...
    */
    ...
    //Original - adapted to use _totalScoreField
    private void displayTotalScore()
    {
        ...
        //TotalOutOf field
        NumberField totalOutOfField = NumberField.getAsUneditable(_rubric.getTotalOutOf());
    ...
    private void displayStatus()
    {
        ...
        layout.putConstraint(SpringLayout.WEST, scoreField, 5, SpringLayout.EAST, statusText);
        layout.putConstraint(SpringLayout.NORTH, scoreField, 0, SpringLayout.NORTH, statusText);
        panel.add(scoreField);
